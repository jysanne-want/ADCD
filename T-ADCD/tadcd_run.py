...
class ADCD_Runner:
    ...
    def _load_meta_and_q_matrix(self):
        ...
        for _, row in p2k_df.iterrows():
            ...
            sids = self._parse_sequence_field(row['skill_ids'])

            if sids and pid < self.p_n:
                valid_sids =[int(sid) for sid in sids if 0 <= int(sid) < self.k_n]
                if valid_sids:
    ...
    def _transform_to_dataloader(self, df, shuffle=True):
        ...
        kc_counts_list = df['kc_counts_parsed'].tolist()
        rt_values = df['rt_norm'].to_numpy(dtype=np.float32).reshape(-1, 1)
        user_speed_values = df['user_speed'].to_numpy(dtype=np.float32).reshape(-1, 1)
        item_load_values = df['item_load'].to_numpy(dtype=np.float32).reshape(-1, 1)

        kc_counts_tensor = torch.from_numpy(np.stack(kc_counts_list)).float()
        rt_tensor = torch.tensor(rt_values, dtype=torch.float32)
        user_speed_tensor = torch.tensor(user_speed_values, dtype=torch.float32)
        item_load_tensor = torch.tensor(item_load_values, dtype=torch.float32)
    ...
    def _run_single_train_eval(self, train_df, val_df, test_df, fold_name=""):
        ...
        model = self.model_class(
            n=self.u_n, m=self.p_n, k=self.k_n, 
            **self.model_kwargs)
        ...
        logging.info(f"--- {fold_name} Best validation AUC: {best_epoch} ---")
