...
class ADCD_Net(nn.Module):
    def __init__(self, n, m, k, dim, emb_dropout=0.3):
        ...
        # 恢复扁平知识点 Embedding，移除知识点层级/领域结构
    ...
    def forward(self, s_id, e_id, q_vec, kc_counts, rt=None, user_speed=None, item_load=None):
        ...
        u_gen = self.delta_generator(theta_s) + student_time_context
        u_spec = self.delta_emb(s_id) + student_time_context
        ...
        gate_input = torch.cat([
                torch.log1p(total_counts.float()),
                user_speed.float(),
                item_load.float(),
                rt.float(),
                user_speed.float() - item_load.float(),
            ],
            dim=1,
        )
